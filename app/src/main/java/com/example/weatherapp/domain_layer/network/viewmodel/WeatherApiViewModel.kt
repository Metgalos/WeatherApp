...
class WeatherApiViewModel @Inject constructor(
    ...
    private fun saveLastWeather(weatherResponse: Response<Weather>) {
        viewModelScope.launch {
            weatherResponse.body()?.let { weather ->
                if (weatherResponse.isSuccessful) {
                    weatherRepository.addFromResponse(weather)
                    weather.location?.name?.let { city -> storage.saveLastCity(city) }
                }
            }
        }
    }
